...
public class Hw1 {
 ...
 abstract class Human{
    ...
    protected int Lfatigue=0;

    Human(String name, int L){
        this.name = name;
        this.Lfatigue=L;
    }
    String get_name(){
 ...
 abstract class Academic extends Human{
    ...
    Academic(String name, int floorId, int classId, int L){
        super(name,L);
 ...
 abstract class Student extends Academic{
    ...
    Student(String name,int floorId,int classId,int L){
        super(name,floorId,classId,L);
...
class Corridor extends Space{
    ...
    public Student exit(){
        Student temp = students.getFirst();
...
class Floor extends Trasplace{
    ...
    public Student exit(){
        for(int i=0;i<6;i++){
            if(this.classrooms[i].available_space()==0)
                continue;
            else{
                Student temp;
                temp = this.classrooms[i].exit();
